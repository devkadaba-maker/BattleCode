...
def favourable_head_attack(controller: Controller, game_state: Game, target: Position) -> bool:
    """Let a short, earlier-acting scout trade itself for an enemy head."""
    if game_state.get_round_num() >= 380 or controller.get_length() > HUNTER_MAX_LENGTH:
        return False
    for tile in controller.get_tiles():
        dragon = tile.get_dragon()
        if dragon is None or dragon.get_team() == controller.get_team() or not dragon.is_head():
...
def pearl_value(controller: Controller, game_state: Game, start: Position) -> int:
    """Prefer nearby pearls, imminent spawns, and routes that deny the opponent."""
    value = 0
    for tile in controller.get_tiles():
        if not tile.has_pearl():
            continue
        d = distance(start, tile.get_position(), game_state)
        value = max(value, max(0, 90 - d * 10))
...
def population_target(game_state: Game) -> int:
    ...
    return min(game_state.get_unit_limit(), max(16, min(32, area // 22 + 16)))


def flagship_target(game_state: Game) -> int:
    return 3 if game_state.width * game_state.height >= 600 else 2
...
def score_move(controller: Controller, game_state: Game, direction: Direction, positions: list[Position]) -> int:
    ...
    if not late and area >= 10 and future_mobility >= 2:
        ...
        score += pearl_value(controller, game_state, target)
...
def split_size(controller: Controller, game_state: Game) -> int:
    ...
    ramped_target = min(target, 4 + max(0, round_number - 8) // 5)
    if (round_number < 8 or round_number >= 380 or controller.get_unit_count() >= ramped_target or
    ...
    if flagship and controller.get_unit_count() >= flagships and controller.get_length() < 24:
        return 0
    minimum_parent = 8 if flagship else 3
    ...
    if best_score == INT_MIN:
        for direction in Direction.get_direction_list():
            ...
            if here is not None and here.get_edge(direction).is_passable() and ahead is not None and not occupied(ahead) and not enemy_head_ahead(ct, target):
                best = direction
                break
    if (not has_empty_step(ct) and game.get_round_num() < 380 and ct.get_unit_count() < population_target(game) and ct.can_split(2) and ct.get_length() >= 6):
        ct.do_split(2)
        return
    sprint = game.get_round_num() < 160 and ct.get_unit_count() < 6 and ct.get_length() >= 10 and safe_sprint(ct, game, best, 2, history)
    if sprint:
