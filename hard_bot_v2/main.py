...
def favourable_head_attack(controller: Controller, game_state: Game, target: Position) -> bool:
    """Let a short hunter trade for a head when turn order makes it reliable."""
    if role_for(controller.get_id()) != ROLE_HUNTER:
        return False
    if controller.get_length() > HUNTER_MAX_LENGTH or game_state.get_round_num() >= 480:
        return False
    if combat_mode(controller, game_state) not in (MODE_PRESSURE, MODE_HUNT, MODE_BALANCED):
        return False
    for tile in controller.get_tiles():
        dragon = tile.get_dragon()
        if dragon is None or dragon.get_team() == controller.get_team() or not dragon.is_head():
...
def role_for(dragon_id: int) -> str:
    """Give v2 a 60/40 hunter/collector mix to pressure specialized swarms."""
    return ROLE_HUNTER if ((role_hash(dragon_id) ^ dragon_id) % 5) < 3 else ROLE_COLLECTOR


def is_flagship(controller: Controller, game_state: Game) -> bool:
    return (role_for(controller.get_id()) == ROLE_COLLECTOR and
...
def score_move(controller: Controller, game_state: Game, direction: Direction, positions: list[Position]) -> int:
    ...
    if role == ROLE_COLLECTOR:
        if area >= 6 and future_mobility >= 2:
            score += pearl_drive(controller, game_state, target) * (2 if flagship else 3)
        score += enemy_pressure(controller, game_state, target)
    else:
        score += enemy_chase_value(controller, game_state, target) * 10
        score += enemy_pressure(controller, game_state, target) // 4
    # Keep a small amount of the old adaptive pressure so v2 still searches
    # when a collector cannot currently see a pearl.
    score += enemy_chase_value(controller, game_state, target) * chase_weight // 2
    if future_mobility == 1:
        score -= 80
    score += friendly_pressure(controller, game_state, target)
    if enemy_collision_risk(controller, game_state, target):
        if role == ROLE_COLLECTOR and late:
            return INT_MIN
        score -= 260 if role == ROLE_HUNTER else 550
    if target.x == 0 or target.y == 0 or target.x == game_state.width - 1 or target.y == game_state.height - 1:
        score -= 220 if role == ROLE_COLLECTOR else 120
...
def split_size(controller: Controller, game_state: Game) -> int:
    ...
    if (round_number < 8 or round_number >= 420 or controller.get_unit_count() >= ramped_target or
        ...
        return 0
    flagship, role = is_flagship(controller, game_state), role_for(controller.get_id())
    ...
    return 2 if controller.can_split(2) and controller.get_length() - 2 >= minimum_parent else 0
